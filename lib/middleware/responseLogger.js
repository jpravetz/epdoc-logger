...
var ResponseLogger = {
    ...
    send: function(body) {
        this.action('respond.send')
            .logObj('status', this.statusCode)
            .logObj('responseTime', this.hrResponseTime());
        if( this.req._delayTime ) {
            this.logObj('delay', this.req._delayTime);
        }
        this.info();
...
// For weird situations, when we want to use the formatter, then get the string back
    ...
    /**
     * Do res.json with a custom built request success object

...
// For weird situations, when we want to use the formatter, then get the string back
     ...
     * @param opt_errorCode {String|Number} Application-specific error, may also be set with res.errorCode(); Defaults to -1 if not specified.
    ...
    fail: function( errorCode, message ) {
        ...
        // Parse parameters
        if( !_.isUndefined(opt_errorCode) ) {
            this._logging.errorCode = opt_errorCode;
        } else {
    ...
    onError: function( err ) {
        ...
        return this.status(500)._origSend(resp);
    }
}
...
var resUtil = {
    ...
    defaultResponseBuilder: function( req, res, opt_err ) {
        ...
        if( !_.isUndefined(opt_err) ) {
            ...
            var msg = (opt_err instanceof Error) ? opt_err.message : JSON.stringify(opt_err);
            var e = {
                status: 500,
                message: msg,
                errorCode: opt_err.code ? opt_err.code : opt_err.name
            };
    ...
    jsonApiResponseBuilder: function( req, res, opt_err ) {
        ...
        if( !_.isUndefined(opt_err) ) {
            ...
            var e = {
                status: 500,
                title: (opt_err instanceof Error) ? opt_err.name : "Server exception",
                detail: msg,
                code: opt_err.code
            };
            if( _.isObject(p.errorParams) ) {

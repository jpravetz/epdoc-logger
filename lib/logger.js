...
/**
 * Return a new logger object instance using the specified module name.
...
function flushQueue() {
    if( aStreams[0] && aStreams[0].ready() ) {
...
module.exports.setLogger = function( type, options ) {
    ...
    if( _.isString(type) ) {
        ...
        Logger = require('./transports/console');
    }
    if( Logger ) {
        var stream = new Logger(options);
        if( stream.validateOptions(aStreams[0]) ) {
            ...
            writeLoggerMessage("warn", "logger.push.warn", ("Unsupported setLogger operation when current logger is " + aStreams[0].type() ), {options: options});
        }
    }
};
 ...
 */
module.exports.getCurrentLogger = function() {
    return aStreams[0];
};

var startLogger = function() {
    if( aStreams[0] ) {
        writeLoggerMessage("info", "logger.start", "Starting '" + stream + "' transport", {stream: stream.toString()});
        aStreams[0].open(onSuccess, onError, onClose);

        function onSuccess() {
            writeLoggerMessage("info", "logger.start.success", "Started '" + stream + "' transport"
        ...
        function onError( err ) {
            writeLoggerMessage("warn", "logger.start.warn", "Tried but failed to set transport to '" + stream + "': " + err);
            unsetLogger();
        };

        function onClose() {
            writeLoggerMessage("info", "logger.start.close", "Transport '" + stream + "' closed");
            unsetLogger();
        }
    } else {
        writeLoggerMessage("warn", "logger.start.error", "Cannot start current transport because no more transports in stack");
    }
};

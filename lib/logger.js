...
module.exports.setLogger = function( name, options ) {
    var bValid = false;
    if( name === 'sos' && aStreams[0].type !== 'sos' )
        bValid = true;
    else if( name === 'file' && aStreams[0].type !== 'sos' && options && options.path )
        bValid = true;

    if( bValid ) {
        var Logger = require('./'+name);
        var stream = new Logger( options );
        writeMessage( "info", "Setting logger to " + stream );
        aStreams[0].end();
        ...
        function onSuccess() {
            stream.clear();
            writeMessage( "info", "Set logger to " + stream );
        };

        function onError(err) {
            writeMessage( "Tried but failed to set logger to " + stream + ": " + err );
            popLogger();
        };

        function onClose() {
            writeMessage( "Logger " + stream + " closed" );
            popLogger();
        }
...
var popLogger = function() {
    ...
    if( aStreams.length > 1 ) {
        ...
        writeMessage( "info", "Restoring previous " + stream + " logger" );
        ...
        writeMessage( "warn", "Aborting popLogger because no more loggers in stack" );
    }

...
var Logger = function (modulename) {
    ...
    this.date = function(d,s) {
        if( self.isAboveLevel('info') ) {
            ...
            self.logObj( { localtime: DateUtil.toISOLocalString(d), utctime: d.toISOString(), uptime: DateUtil.formatMS(d-t0) } );
            self.logArgs( 'info', [] );
...
module.exports.setLogger = function( options ) {
    ...
    if( bValid ) {
        ...
        function onError(err) {
            writeLoggerMessage( "error", "logger.push.error", "Tried but failed to set logger to " + stream + ": " + err );
            popLogger();
        };
        ...
        writeLoggerMessage( "error", "logger.push.error", ("Unsupported setLogger operation when current logger is " + aStreams[0].type), { options: options } );
    }
};
...
var popLogger = function() {
    ...
    if( aStreams.length > 1 ) {
        ...
        function onError(err) {
            writeLoggerMessage( "error", "logger.pop.error", "Tried but failed to set logger to " + stream + ": " + err );
            popLogger();
        };
        ...
        writeLoggerMessage( "warn", "logger.pop.abort", "Aborting popLogger because no more loggers in stack" );
    }

...
//			writeMessage( logMsg );
    ...
    this._writeMessage = function( level, msg ) {
        ...
        if( args.length > 1 ) {
            var tnow = new Date();
            var params = { module: this.moduleName, time: tnow, timeDiff: (tnow.getTime()-t0) };
            params.level = args.shift();
            if( gGetSessionIdCallback && args.length > 1 && (typeof args[0] === 'object') && ( typeof args[0][gGetSessionIdCallback] === 'function') ) {
                var obj = args.shift();

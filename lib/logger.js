...
var Logger = function (name) {
	...
	this.date = function() {
		...
		if( this.isAboveLevel('info') ) {
            ...
            oStream.write( this.moduleName, 'info', msg, timeDiff() );
...
//			writeMessage( logMsg );
	...
	this.log = function( level, msg ) {
		if( this.isAboveLevel(level) ) {
            oStream.write( this.moduleName, level, msg, timeDiff() );
...
//			writeMessage( logMsg );
    ...
    oStream = oStream || require('./console')();
 ...
 */
exports.setLogger = function( name, timeout) {
    ...
    if( name === 'sos' ) {
        var SOS = require('./sos');
        newStream = new SOS();
        newStream.open( function(bSuccess) {
            console.log( 'result = %d', bSuccess );
            if( bSuccess ) {
                oStream.write( "logger", "info", "Setting logger to SOS", timeDiff() );
                oStream = newStream;
                oStream.clear();
                oStream.write( "logger", "info", "Setting logger to SOS", timeDiff() );
                result = true;
            } else {
                oStream.write( "logger", "info", "Tried but failed to set logger to SOS", timeDiff() );
                result = false;
            }
        });
    } else {
    ...
    if( result === undefined ) {
        ...
        timer = setTimeout( function() {
            if( newStream )
                newStream.close();
            result = name === 'sos' ? false : true;
            console.log( "Timeout on %s connection", name);
        }, timeout );
    }
